...
#                   select(advertiser_id, Candidate))  %>% 
#   filter(under100) %>% 
#   filter(Candidate == "VVD")
...
#     # filter(Candidate == "SP")
#     ggplot(aes(date_produced, spend, color = Candidate)) +
#     geom_line(size = 1.5) +
#     scale_color_parties() +
...
#   clipr::write_clip()
#   hchart('area', hcaes(x = 'date_produced', y = 'spend', group = "Candidate"))
...
# color_dat <- color_dat %>% mutate(Candidate  =coalition)
...
hc_data <-  more_data %>%
  # mutate(advertiser_id = as.character(advertiser_id)) %>%
    ...
    summarize(spend  = sum(spend)) %>%
    ungroup() %>%
  # spread(key = Candidate, value = spend, fill = 0) %>%
  # arrange(date_produced) %>%
  ...
  # pivot_longer(-date_produced, names_to = "Candidate", values_to = "spend")  %>%
  group_by(Candidate) %>%
  mutate(total_spend = max(spend)) %>%
  ungroup()  %>%
  left_join(color_dat, by = "Candidate") %>%
