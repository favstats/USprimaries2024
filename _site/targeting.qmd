...
How much did campaigns spend on different targeting methods? The graphs below show spending *overall* across all parties and *by Candidate*.
...
get_contested_graph(election_dat30, 5000)
```
...
## Static version
tm <- treemap(region_dat, index = c("Candidate", "region"),
...
```{r, fig.width= 8, fig.height=5, dpi = 300, eval =F, results='asis'}
...
<!--      #                              "Coalizione di centro-destra"))) %>%  -->
<!--     mutate(value = fct_reorder(value, perc)) %>%  -->
<!--     ggplot(aes(value, perc, fill = Candidate)) + -->
<!--     geom_col(alpha = 0.8) + -->
